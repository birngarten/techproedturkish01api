...
//		SORU : ilk 5 ismin Tiger Nixon, Garrett Winters, Ashton Cox, Cedric Kelly, Airi Satou old. 'verify' et
		...
		isimList.add("Airi Satou");	
			...
			softAssert.assertEquals(json.getList("data[0,1,2,3,4].employee_name" ),isimList);			
//			***	45. satir ile for loop ayni isi yapar
			for(int i=0; i<isimList.size(); i++) {
				softAssert.assertEquals(json.getString("data[" + i + "].employee_name"), isimList.get(i));
			
		}
		
		// 3.YOL tavsiye edilir (En iyi yol)
